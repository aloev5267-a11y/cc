...
// Contact API request (общая форма + заявка из калькулятора доставки)
export const contactRequestSchema = z.object({
  ...
  source: z.enum(['contact_form', 'delivery_calc']).default('contact_form'),
  // Доп. данные из калькулятора доставки
...
export const deliveryFormSchema = z.object({
  fromCity: z.string().min(1, 'Укажите город отправления'),
  toCity: z.string().min(1, 'Укажите город назначения'),
  weight: z.number().positive('Вес должен быть положительным').max(30000, 'Максимальный вес 30 тонн'),
  type: z.enum(['documents', 'package', 'cargo']),
})
